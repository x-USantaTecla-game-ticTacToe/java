...
class UserPlayerView extends PlayerView {
    ...
    UserPlayerView(Logic logic, boolean isMovement, Container panel) {
        super(logic);
        if (isMovement) {
            this.coordinateMoveView = new CoordinateMoveView(logic);
            panel.add(this.coordinateMoveView, new Constraints(0, 1, 3, 1));
        } else {
            this.coordinatePutView = new CoordinatePutView(logic);
